 ...
 * environment versions ever leave the machine. The API key, queried origins
 * and URLs, tool arguments, prompts and the names or values of environment
 ...
 * Opt out for every server by this author at once: ASKADS_TELEMETRY=0
 ...
 * counterpart — the server now survives a missing CRUX_API_KEY and completes
 * the handshake, so this is the first event that can carry the client's name
 * for an install that has no key in yet.
 */
